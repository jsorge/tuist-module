...
extension ModuleName {
    /// The test target name for a given module (i.e. ModuleName.MyModule.tests produces `MyModuleTests`)
    public var tests: ModuleName {
        return ModuleName(name: "\(self.name)Tests")
    }

    /// The resource bundle name for a given module (i.e. ModuleName.MyModule.resources produces
    /// `MyModuleResources`)
...
extension String {
    ...
    public static func bundleID(from module: ModuleName) -> String {
        #warning("Need to have a custom bundle base ID injected")
        return module.name
